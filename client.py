        ...
        while True:
            ...
            while i is None:
                ...
                for i, dialog in enumerate(dialogs, start=1):
                    ...
                    #sprint('{}. {}'.format(i, get_display_name(dialog.entity)))!q
                    entity = await client.get_entity(dialog.entity)
                    #print(entity)
                    #try:
                    if not hasattr(entity, 'username'):
                        if not hasattr(entity,'megagroup'):
                            #conn.execute('''INSERT INTO users (id,uid,nname, fname, lname,phone, type) VALUES (?,?,?,?,?,?,?)''',
                            #(i,entity.id,entity.title, '', '','','lgroup'))
                            #conn.commit()
                            print("--")
                        else:
                            #conn.execute('''INSERT INTO users (id,uid,nname, fname, lname,phone, type) VALUES (?,?,?,?,?,?,?)''',
                            #(i,entity.id,entity.title, '', '','','mgroup'))
                            #conn.commit()
                            print(" ")
                    elif hasattr(entity, 'title'):
                        #conn.execute('''INSERT INTO users (id,uid,nname, fname, lname,phone, type) VALUES (?,?,?,?,?,?,?)''',
                        #(i,entity.id,entity.username, entity.title, '','','channel'))
                        #conn.commit()
                        print(" ")
                    elif hasattr(entity, 'first_name'):
                        if (entity.bot):
                            #conn.execute('''INSERT INTO users (id,uid,nname, fname, lname,phone, type) VALUES (?,?,?,?,?,?,?)''',
                            #(i,entity.id,entity.username, entity.first_name, entity.last_name,entity.phone,'bot'))
                            #conn.commit()
                            print(" ")
                        else:
                            #conn.execute('''INSERT INTO users (id,uid,nname, fname, lname,phone, type) VALUES (?,?,?,?,?,?,?)''',
                            #(i,entity.id,entity.username, entity.first_name, entity.last_name,entity.phone,'user'))
                            #conn.commit()
                            sprint('{}. {}'.format(i, get_display_name(dialog.entity)))
                ...
                print()    
                print("Done!!!")
                    #sprint('{}. {}'.format(i, entity.id))
                # Let the user decide who they want to talk to
                print()
                #print('> Who do you want to send messages to?')
                #print('> Available commands:')
                print('  !q: Quits the dialogs window and exits. Or NUM to write to.')
                #print('  !l: Logs out, terminating this session.')
                print()
                
                i = await async_input('Wait for a command: ')
                if i == '!q':
                    #conn.close()
                    return
                if i == '!l':
            ...
            while True:
                ...
                # History
                    ...
                    for msg in reversed(messages):
                        ...
                        sprint('[{}:{}] (ID={}) {}: {}'.format(
                            msg.date.hour+3, msg.date.minute, msg.id, name, content))
    ...
    async def message_handler(self, event):
        ...
        if event.is_group:
            if event.out:
                ...
                # Colloquium, Беседка, то есть группы
                sprint('<-< {} >>>>{} -> {}'.format(
            ...
            if event.out:
               ...
               #('me',get_display_name(chat), event.text, get_display_name(chat),event.id))
                ...
                realport.write(event.text.encode())
                #conn.execute('''INSERT INTO mess (u_from, u_to, mess,chat, idm) VALUES (?,?,?,?,?)''',
